...
Public Class Frm_StandbyServer
    ...
    Private Sub BackgroundWorker_DoWork(sender As System.Object, e As System.ComponentModel.DoWorkEventArgs) Handles BackgroundWorker.DoWork
        Try
            SocketServer.Bind(New IPEndPoint(IPAddress.Any, StandbyServerPort)) '绑定服务器地址
            While True
                ...
                Select Case ReceiveBuffer.Order
                    Case "Login"        '用于处理用户登录请求
                        ...
                        Else 'If isFound = True Then
                            ...
                            SocketServer.SendTo(SerializeMessage(SendBuffer), ClientPoint)
                        End If
                    Case "Update"       '用于处理用户更新信息请求
                        '返回成功建立连接确认
                        SendBuffer.Order = "UpdateSuccessfully"
                        SocketServer.SendTo(SerializeMessage(SendBuffer), ClientPoint)
                    Case "Validate"         '用于处理用户远程验证请求
                        ...
                        Try
                            ...
                            Connection.Open()
                            Dim SQL As String = "Select * From [Student] Where [StudentID]=" & ReceiveBuffer.StudentID
                            Dim Command = New OleDbCommand(SQL, Connection)
                            Dim Reader As OleDbDataReader = Command.ExecuteReader
                            While Reader.Read
                                Count += 1
                                If ReceiveBuffer.StudentID = Trim(Reader(("StudentID"))) Then
                                    '验证用户信息是否正确
                                    If ReceiveBuffer.UserPassword = Trim(Reader("Password")) Then
                                        '读取用户信息
                                        SendBuffer.StudentID = Trim(Reader("StudentID"))
                                        SendBuffer.StudentName = Trim(Reader("StudentName"))
                                        SendBuffer.Group = Trim(Reader("Group"))
                                        '提示用户验证成功
                                        SendBuffer.Order = "ValidateSuccessfully"
                                        SocketServer.SendTo(SerializeMessage(SendBuffer), ClientPoint)
                                        '写入系统日志
                                        ListLog.Items.Add(System.DateTime.Now.ToLongTimeString & " : 学号 " & ReceiveBuffer.StudentID & "(" & Trim(Reader("StudentName")) & ")的用户在" & ClientPoint.ToString & " 验证用户有效性: 成功")
                                    Else
                                        '提示用户验证失败
                                        SendBuffer.Order = "ValidateFailed"
                                        SocketServer.SendTo(SerializeMessage(SendBuffer), ClientPoint)
                                        '写入系统日志
                                        ListLog.Items.Add(System.DateTime.Now.ToLongTimeString & " : 学号 " & ReceiveBuffer.StudentID & "(" & Trim(Reader("StudentName")) & ")的用户在" & ClientPoint.ToString & " 验证用户有效性: 失败")
                                    End If
                                    Exit While
                                End If
                            End While
                            ...
                            If Count = 0 Then
                                ...
                                SocketServer.SendTo(SerializeMessage(SendBuffer), ClientPoint)
                                '写入系统日志
                                ListLog.Items.Add(System.DateTime.Now.ToLongTimeString & " : 学号 " & ReceiveBuffer.StudentID & "的用户在" & ClientPoint.ToString & " 验证用户有效性: 失败")
                            End If
                        Catch ex As Exception
                            MsgBox("An error occured while Validating [UserInformation]." & Chr(13) & Chr(13) & "[Exception] " & ex.Message)
                        End Try
                    Case "ConnectTest"  '用于处理报名系统测试连接请求
                        '返回连接正常信息
                        SendBuffer.Order = "Connected"
                        SocketServer.SendTo(SerializeMessage(SendBuffer), ClientPoint)
                        '写入系统日志
                        ListLog.Items.Add(System.DateTime.Now.ToLongTimeString & " : IP地址为" & ClientPoint.ToString & " 的计算机检查连接可用性: 可用.")
                    Case "Register"     '用于处理用户提交报名信息请求
                        '检查数据库文件是否存在
                        If Dir(DatabaseLocation) = Nothing Then
                            MsgBox("未找到数据库文件,请与管理员联系!" & Chr(13) & "对您造成不便我们深表歉意!" & Chr(13) & Chr(13) & "[Exception] Database Not Found!")
                            Exit Sub
                        ...
                        Try
                            ...
                            While Reader.Read
                                If ReceiveBuffer.StudentID = Trim(Reader("StudentID")) Then
                                    SendBuffer.Order = "UserRegistered"
                                    SocketServer.SendTo(SerializeMessage(SendBuffer), ClientPoint)
                                    Exit Sub
                                End If
                            End While
                            Command.Dispose()
                            '在本地数据库中记录用户信息
                            SQL = "Insert into [Student] values (" & ReceiveBuffer.StudentID & ", '" & ReceiveBuffer.StudentName & "', '" & ReceiveBuffer.UserPassword & "', '" & ReceiveBuffer.Group & "', '" & ReceiveBuffer.School & "', '" & ReceiveBuffer.Profession & "', '" & ReceiveBuffer.PhoneNumber & "')"
                            Command = New OleDbCommand(SQL, Connection)
                            Command.ExecuteNonQuery()
                            Connection.Close()
                            '通知用户已完成报名
                            SendBuffer.Order = "RegisterSuccessfully"
                            SendBuffer.UserPassword = ReceiveBuffer.UserPassword
                            SocketServer.SendTo(SerializeMessage(SendBuffer), ClientPoint)
                            '写入系统日志
                            ListLog.Items.Add(System.DateTime.Now.ToLongTimeString & " : 学号 " & ReceiveBuffer.StudentID & "(" & ReceiveBuffer.StudentName & ")的用户在" & ClientPoint.ToString & " 完成注册.")
                        Catch ex As Exception
                    ...
                    Case "Logout"       '用于处理用户退出登录请求
                        ...
                        If isFound = True Then
                            ...
                            End If
                        End If
                    Case "SyncWithMainServer"           '接收来自主服务器的同步信息
                        SecondDown = ReceiveBuffer.TimeUsed
                    Case "SyncWithStandbyServer"        '发送信息使主服务器与本机同步
    ...
    Private Sub ButtonSynchronize_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonSynchronize.Click
        Dim MainServerPoint As New IPEndPoint(IPAddress.Parse(MainServerIpAddress), MainServerPort)
        SendBuffer.Order = "SyncWithMainServer"
        SocketServer.SendTo(SerializeMessage(SendBuffer), MainServerPoint)
    End Sub
        ...
        If ListLogin.SelectedItems.Count <> 0 And ListLogin.Items.Count > 0 Then
            For Each Item As ListViewItem In ListLogin.SelectedItems
                If MsgBox("您确定要删除学号为 " & Item.SubItems(0).Text & "(" & Item.SubItems(1).Text & ")" & " 用户吗?", vbYesNo) = vbYes Then
                    ...
                    End If
                    '写入系统日志
                    ListLog.Items.Add(System.DateTime.Now.ToLongTimeString & " : 学号 " & Item.SubItems(0).Text & " 的用户(" & Item.SubItems(1).Text & ") 被管理员手动登出")
                End If
            Next
        Else
