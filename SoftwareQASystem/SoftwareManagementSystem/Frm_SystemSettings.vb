...
Public Class Frm_SystemSettings
    ...
    Private Sub Frm_SystemSettings_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        Else
            ButtonEnter_Click(sender, e)    '自动登录
        End If
    End Sub
    ...
    Private Sub ButtonReset_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonReset.Click
        ...
        End If
    End Sub
    Private Sub ButtonViewQuestionLibrary_Click(ByVal sender As Object, ByVal e As EventArgs) Handles ButtonViewQuestionLibrary.Click
        Frm_QuestionLibrary.ShowDialog()
    End Sub
    Private Sub MenuItemMainServer_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MenuItemMainServer.Click
        If isLogined = True Then
            Frm_MainServer.Show()
            Me.Close()
        Else
            MsgBox("请先登录后才能启动主服务器!" & Chr(13) & "您也可以在用户登陆页面登录.")
            Exit Sub
        End If
    End Sub
    Private Sub MenuItemStandbyServer_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MenuItemStandbyServer.Click
        If isLogined = True Then
            Frm_StandbyServer.Show()
            Me.Close()
        Else
            MsgBox("请先登录后才能启动备用服务器!" & Chr(13) & "您也可以在用户登陆页面登录.")
            Exit Sub
        End If
    End Sub
    ...
    Private Sub MenuItemImportSettings_Click(ByVal sender As Object, ByVal e As EventArgs) Handles MenuItemImportSettings.Click
        If isLogined = False Then
            MsgBox("请先登录后才能导入配置文件!" & Chr(13) & "您也可以在用户登陆页面登录.")
            Exit Sub
        Else
            If ImportExportFile("Import", "Config.Xml") Then
                MsgBox("已成功导入新的配置文件!")
            End If
        End If
        '从导入的Config.Xml文件中读取配置信息
        Dim XmlDocumentPath = Application.StartupPath & "\Config.Xml"
        If Dir(XmlDocumentPath) <> Nothing Then
            ReadXmlDocument(XmlDocumentPath)
        End If
    ...
    Private Sub MenuItemImportDatabase_Click(ByVal sender As Object, ByVal e As EventArgs) Handles MenuItemImportDatabase.Click
        If isLogined = False Then
            MsgBox("请先登录后才能导入数据库文件!" & Chr(13) & "您也可以在用户登陆页面登录.")
            Exit Sub
        Else
            If ImportExportFile("Import", "DataBase.mdb") Then
                MsgBox("已成功导入新的数据库文件!")
            End If
        End If
    End Sub
    Private Sub MenuItemExportSettings_Click(ByVal sender As Object, ByVal e As EventArgs) Handles MenuItemExportSettings.Click
        If isLogined = False Then
            MsgBox("请先登录后才能导出配置文件!" & Chr(13) & "您也可以在用户登陆页面登录.")
            Exit Sub
        Else
    ...
    Private Sub MenuItemExportDatabase_Click(ByVal sender As Object, ByVal e As EventArgs) Handles MenuItemExportDatabase.Click
        If isLogined = False Then
            MsgBox("请先登录后才能导出数据库文件!" & Chr(13) & "您也可以在用户登陆页面登录.")
            Exit Sub
        Else
